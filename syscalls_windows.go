...
func getdeviceids(componentID string) ([]string, error) {
	k, err := registry.OpenKey(registry.LOCAL_MACHINE, tapDriverKey, registry.READ)
	if err != nil {
		return []string{}, fmt.Errorf("Failed to open the adapter registry, TAP driver may be not installed, %v", err)
	}
	defer k.Close()
	...
	for _, v := range keys {
		...
		if val == componentID {
			...
			key.Close()
			deviceids = append(deviceids,val)
		}
		key.Close()
	}
	if len(deviceids) == 0 {
		return []string{}, fmt.Errorf("Failed to find the tap device in registry with specified ComponentId '%s', TAP driver may be not installed", componentID)
	} else {
...
func setTUN(fd syscall.Handle, network string) error {
    ...
    code1  := make([]byte, 0, 16)
	code1 = append(code1,localIP.To4()[:4]...)
	code1 = append(code1, remoteNet.Mask[:4]...)
	code1 = append(code1, remoteNet.IP.To4()[:4]...)
	code1 = append(code1, 16, 14, 0, 0) // For 1 Hour
//	code1 := []byte { 10, 0, 0, 2, 255, 255, 255, 0, 10, 0, 0, 1, 1, 1, 81, 128 }
	if err := syscall.DeviceIoControl(fd, tap_win_ioctl_config_dhcp_masq, &code1[0], uint32(16), &rdbbuf[0], uint32(len(rdbbuf)), &bytesReturned, nil); err != nil {
		return err
	}
...
func openDev(config Config) (ifce *Interface, err error) {
	// find the device in registry.
	deviceids, err := getdeviceids(config.PlatformSpecificParams.ComponentID)
	if err != nil {
		return nil, err
	}
